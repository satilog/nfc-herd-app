    ...
    <div className="h-full">
      ...
      <div className="relative h-full">
        <input
          ...
          className="pl-8 pr-4 py-1 bg-black text-white border border-white rounded"
        />
        <FiSearch
      ...
      {isPopupOpen && (
        <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-90 z-10 flex items-center justify-center">
          <FiX
            className="absolute top-5 right-5 text-white cursor-pointer"
            size={30}
            onClick={() => setIsPopupOpen(false)}
          />
          <div className="custom-container relative flex flex-col w-full h-full max-h-screen py-20 px-10 bg-black overflow-y-auto">
            <form onSubmit={handleSearch} className="relative mb-10">
              <input
                ...
                className="pl-8 pr-4 py-2 w-full bg-black text-white border-2 border-white rounded"
              />
              <FiSearch
                className="absolute top-1/2 left-2 transform -translate-y-1/2 text-white"
                size={15}
            ...
            <div className="flex flex-col gap-4">
              {[...Array(20)].map((item, index) => (
                <div className="bg-white text-black p-5 rounded" key={index}>
                  <h1>Result Title</h1>
