...
export default function CattleList() {
  ...
  const fetchCattles = async (farmId: string) => {
    try {
      ...
      const parsedCattlesData = cattlesData.map((cattleString: any) => JSON.parse(cattleString));
      ...
      <div className="relative mb-4">
        {farm && (
          <div className="mt-4 mb-8">
            <h2 className="text-3xl font-bold text-left text-gray-800 mb-2">{farm.name}</h2>
            <p className="text-md text-left text-gray-600">Location: {farm.location}</p>
            <p className="text-md text-left text-gray-600">Size: {farm.size_acres} acres</p>
          </div>
        )}
        <h1 className="text-2xl font-bold text-left text-gray-800">Cattle in farm</h1>
      </div>
      {isLoading ? (
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          {cattles.map((cattle) => (
            ...
            >
              <div className="flex items-center text-gray-800">{cattle.breed}</div>
              ...
              <div className="flex items-center text-gray-800">{calculateAge(cattle.date_of_birth)}</div>
            </div>
          ))}
        </div>
      )}
